...
export function UserChatView({ conversation, currentUser, onSendMessage, allUsers, onBack }: UserChatViewProps) {
  const scrollAreaRef = useRef<HTMLDivElement>(null);

  const getHeaderInfo = () => {
    const otherUser = conversation.participants.find(p => p.role === 'user');
    return {
        avatar: otherUser?.avatar || conversation.icon,
        color: otherUser?.avatarColor || conversation.avatarColor,
        title: otherUser?.name || conversation.title,
