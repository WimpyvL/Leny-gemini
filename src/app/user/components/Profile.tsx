...
import { upgradeToExpert } from '@/app/auth/actions';
...
export function Profile({ user: initialUser }: ProfileProps) {
  ...
  const handleUpgrade = async () => {
    setIsUpgrading(true);
    const result = await upgradeToExpert(user.id);
    if (result.success) {
      toast({
        title: 'Account Upgraded!',
        description: 'You are now registered as an expert. Redirecting...',
      });
      router.refresh();
    } else {
      toast({
                ...
                <div>
                  ...
                  <div className="space-y-2">
                      <div className="flex items-center justify-between rounded-lg border p-3 shadow-sm">
                          <div>
                              <Label className="font-medium">Register as an Expert/Consultant</Label>
                              <p className="text-xs text-muted-foreground">Access expert dashboard and tools.</p>
                          </div>
                          <AlertDialog>
                              ...
                              <AlertDialogContent>
                                  <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                          Are you sure you want to register as an Expert? This will grant you access to expert-level features. This action cannot be easily undone.
                                      </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
