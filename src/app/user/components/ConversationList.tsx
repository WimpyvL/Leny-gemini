...
import { mockRecentSearches, mockFavoriteActions } from '@/lib/mock-data';
...
function ConversationItem({
  ...
  const otherUser = conv.participants.find(p => p.id !== 'user1');
    ...
    >
      ...
      <div className="w-full overflow-hidden">
        <div className="flex justify-between items-baseline">
            ...
            <p className="text-xs text-muted-foreground whitespace-nowrap">{hasMounted ? relativeTime : ''}</p>
        </div>
...
export function ConversationList({ conversations, selectedConversationId, onSelectConversation, onInviteClick }: ConversationListProps) {
  ...
  const handleMicClick = () => {
    const saniConversation = conversations.find(c => c.participants.some(p => p.id === 'assistant'));
    if (saniConversation) {
      onSelectConversation(saniConversation.id);
      setIsSearchActive(false); // Close the search view if it's open
    } else {
      console.log("S.A.N.I. conversation not found.");
    }
  };
        ...
        <div className="relative flex items-center gap-2">
          ...
          <div className="relative w-full">
            ...
            >
                ...
                <span className="sr-only">Chat with S.A.N.I.</span>
            </Button>
          </div>
        </div>
      </div>
